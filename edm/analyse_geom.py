    ...
    for smiles, sub_dic in drugs_summ.items():
        if "pickle_path" in sub_dic:
            pickle_path = os.path.join(
                args.data_dir, "rdkit_folder", sub_dic["pickle_path"]
            )
...
def analyse_geom_stability():
    ...
    num_atoms_total = 0
    formatted_data = []
    for i, (p, at_types) in tqdm(
        enumerate(zip(all_positions_split, all_atom_types_split))
    ):
        ...
        mol_is_stable, num_stable_atoms, num_atoms = check_stability(
            p, at_types, dataset_info
        )
        num_mols += 1
        ...
        if i % 5000 == 0:
            print(
                f"IN PROGRESS -- Stable molecules: {num_stable_mols} / {num_mols}"
    ...
    print(
        f"Stable molecules: {num_stable_mols} / {num_mols} = {num_stable_mols / num_mols * 100} %"
    )
    print(
