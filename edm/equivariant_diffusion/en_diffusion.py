...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_from_z(
        self, z, node_mask, edge_mask, context, fix_noise=False, no_grad=False
    ):
        diffusion_utils.assert_mean_zero_with_mask(z[:, :, : self.n_dims], node_mask)
        # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
        for s in reversed(range(0, self.T)):
            s_array = torch.full((z.shape[0], 1), fill_value=s, device=z.device)
            t_array = s_array + 1
            s_array = s_array / self.T
            t_array = t_array / self.T
            if no_grad:
                with torch.no_grad():
                    z = self.sample_p_zs_given_zt(
                        s_array, t_array, z, node_mask, edge_mask, context, fix_noise
                    )
            else:
                z = checkpoint(
    ...
    def sample_from_z_eps(self, z, eps, node_mask, edge_mask, context):
        diffusion_utils.assert_mean_zero_with_mask(z[:, :, : self.n_dims], node_mask)
        # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
        for s in reversed(range(0, self.T)):
            s_array = torch.full((z.shape[0], 1), fill_value=s, device=z.device)
    ...
    def diffusion_chain(self, x, h, node_mask, keep_frames=100):
        if len(node_mask.shape) == 2:
            node_mask = node_mask[:, :, None]
        xh0 = torch.cat([x, h["categorical"], h["integer"]], dim=2)
        xh_list = [xh0]

        x, h, delta_log_px = self.normalize(x, h, node_mask)
        xh = torch.cat([x, h["categorical"], h["integer"]], dim=2)

        t_int = torch.linspace(0, self.T, steps=keep_frames, device=x.device)
        # Normalize t to [0, 1].
        t = t_int / self.T
        gamma_t = self.inflate_batch_array(self.gamma(t), x).to(x.device)
        alpha_t = self.alpha(gamma_t, x)
        ...
        for i in range(1, t_int.shape[0]):
            eps = self.sample_combined_position_feature_noise(
                n_samples=x.shape[0], n_nodes=x.size(1), node_mask=node_mask
            )
            ...
            s_s = sigma_t[i - 1]
            a_ts = a_t / a_s
            xh = a_ts * xh + (s_t**2 - a_ts**2 * s_s**2) * eps
            xh_list.append(self.unnormalize_z(xh, node_mask))

        return torch.cat(xh_list)
